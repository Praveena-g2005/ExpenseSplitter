...
object Validators {
    ...
    def validateEmail (email: String) : ValidationResult ={
        ...
        if(trimmedemail.isEmpty){
            ...
            ValidationFailure(s"Email should be less than $MAX_EMAIL_LENGTH characters")
    ...
    def validateName (name :String): ValidationResult ={
        val sanitizedname = sanitizedName(name)

        if(sanitizedname.isEmpty){
            ValidationFailure("Name cannot be empty")
        }else if(sanitizedname.length >MAX_NAME_LENGTH){
            ValidationFailure(s"Name of characters should not exceed $MAX_NAME_LENGTH")
        }else{
            ValidationSuccess
        }
    }

    def validateAmount(amount :Double ,fieldname :String= "amount"):ValidationResult ={
        if(amount<=0){
    ...
    def sanitizedEmail(email :String) :String ={
