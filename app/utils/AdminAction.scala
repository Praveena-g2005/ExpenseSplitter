    ...
    extractToken(request) match {
      case Some(token) =>
        authService.validateAccessToken(token).flatMap {
          case Some((userId, email, role)) if role == UserRole.ADMIN.toString =>
            val user = app.models.User(Some(userId), "Unknown", email, "", app.models.UserRole.withName(role))
      ...
      case None =>
        Future.successful(Unauthorized(Json.obj("error" -> "Missing authorization token")))
    }
