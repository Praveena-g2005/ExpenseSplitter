...
class RefreshTokenRepository @Inject()(
  ...
  def create(refreshToken: RefreshToken): Future[RefreshToken] = {
    val insertQuery = refreshTokens returning refreshTokens.map(_.id) += refreshToken
  ...
  def revokeToken(token: String): Future[Int] = {
    val query = refreshTokens.filter(_.token === token).map(_.revoked).update(true)
    db.run(query)
  }
  
  def revokeUserTokens(userId: Long): Future[Int] = {
    val query = refreshTokens.filter(_.userId === userId).map(_.revoked).update(true)
    db.run(query)
  }
  
  def deleteExpiredTokens(): Future[Int] = {
    val now = Timestamp.valueOf(LocalDateTime.now())  // Convert to Timestamp
