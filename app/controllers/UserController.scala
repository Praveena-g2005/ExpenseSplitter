...
import app.utils.{AdminAction, AuthAction, AuthenticatedRequest}
...
class UserController @Inject() (
  ...
  def getAllUsers(): Action[AnyContent] = adminAction.async { request: AuthenticatedRequest[AnyContent] =>
    ...
    userService
      ...
      .recover {
        case ex: Exception =>
          ...
          InternalServerError(
            Json.toJson(ErrorResponse("Failed Fetching users"))
          )
      }
  }

  def getUserById(id: Long): Action[AnyContent] = adminAction.async { request: AuthenticatedRequest[AnyContent] =>
    ...
    userService
      ...
      .map {
        ...
        case None =>
          ...
          NotFound(
            Json.toJson(ErrorResponse(s"User not found with id : $id"))
          )
      }
      .recover {
        case ex: Exception =>
