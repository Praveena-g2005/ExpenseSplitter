...
class RevokedTokenTable(tag: Tag) extends Table[RevokedToken](tag, "revoked_tokens") {
  ...
  def userFk = foreignKey("fk_revoked_user", userId, TableQuery[UserTable])(_.id)
  def idx = index("idx_token", token) 
  
  def * : ProvenShape[RevokedToken] =
    (id.?, token, userId, tokenType, revokedAt, expiresAt) <> (RevokedToken.tupled, RevokedToken.unapply)
}